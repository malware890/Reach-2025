...
def get_cpu(algo):
    ...
    match algo:
        ...
        case "sjf":
            threads_cpu = [NoteThread(yob_threads_cpu[i], i + 4) for i in range(len(yob_threads_cpu))] + [
                NoteThread(mat_thread, 9),
                NoteThread(prime_thread, 10),
                NoteThread(dfs_thread, 1),
                NoteThread(bfs_thread, 3),
                NoteThread(nn_thread, 0),
        ...
        case "priority_schedule":
            threads_cpu = [NoteThread(yob_threads_cpu[i], 6 - i) for i in range(len(yob_threads_cpu))]
            threads_cpu += [
